...
/*
 * The firmware_attributes_class is defined in drivers/base/firmware_loader/fallback.c
 * but the header is not always available in all kernel versions or might be
 * named differently. For most platform drivers, they use the class directly
 * if it is exported.
 *
 * DISABLED: Not exported in Ubuntu kernel 6.14.0-37
 */
// extern struct class firmware_attributes_class;
...
#define GB_ACPI_NOTIFY_HOTKEY_KBD_BACKLIGHT     0x7d  /* Fn+F9 on Galaxy Book5 Pro */
...
static int galaxybook_fw_attrs_init(struct samsung_galaxybook *galaxybook)
{
	...
	/* Firmware attributes disabled - firmware_attributes_class not available in Ubuntu kernel */
	return 0;

	galaxybook->fw_attrs_dev = device_create(NULL, NULL, MKDEV(0, 0),
						 NULL, "%s", DRIVER_NAME);
